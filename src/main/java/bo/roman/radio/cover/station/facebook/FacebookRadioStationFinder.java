...
public class FacebookRadioStationFinder implements RadioStationFindable {
	...
	/**
	 * Retrieve all the radio stations sending a request to Facebook. Radio stations
	 * to be considered will be all the stations that match the name of the radio
	 * name and which categories are: - Radio Station - News - Media - Broadcasting
	 * 
	 * @param radioName
	 * @return
	 */
	private List<Radio> findAllRadioPages(String radioName) {
		...
		if (optResponse.isPresent()) {
			...
			List<Radio> radiosFound = parsedRadios.getData().stream()
					...
					.filter(Radio::hasLogo).peek(r -> r.setName(StringEscapeUtils.unescapeJava(r.getName())))
	...
	/**
	 * From all the Radio Pages, filter them by exact name and return it.
	 * 
	 * If that is not the case, find the radio which name is closely related to the
	 * expected radio.
	 * 
	 * @param radios
	 * @return
	...
	private Optional<Radio> findBestRadio(List<Radio> radios, String radioName) {
		...
		Map<PhraseMatch, List<Radio>> radioGroups = radios.stream().collect(
				Collectors.groupingBy(r -> PhraseCalculator.phrase(radioName).calculateSimilarityTo(r.getName())));
	...
	/**
	 * Find radio by match category and give priority to the face pages that
	 * correspond to the categories listed in: - Radio Station - News - Media -
	 * Broadcasting
	 * 
	 * @param radioGroup
	 * @param match
	...
	private Optional<Radio> findRadioByMatch(Map<PhraseMatch, List<Radio>> radioGroup, PhraseMatch match) {
		...
		if (radioGroup.containsKey(match) && !radioGroup.get(match).isEmpty()) {
			Optional<Radio> aMatch = radioGroup.get(match).stream().sorted((r1, r2) -> {
	...
	/**
	 * Is a valid category if the Radio.Category is similar to: - Radio Station -
